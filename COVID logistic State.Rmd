...
text(max(temp$x)-80, max(temp$y) -6000, paste("Daily new cases = ", new_cases )) 
text(max(temp$x)-80, max(temp$y)-2000, paste("Daily percent increase = ", round(one_day_percent,1), "%" )) 
text(max(temp$x)-80, max(temp$y)-4000, paste("Seven-day percent increase = ", round(seven_days_percent,1), "%" )) 
...
r <- r + geom_vline(xintercept=St[St$Date=="2020-08-18",55],
               linetype=4, colour="red") +
  annotate("text", x = as.Date("2020-08-5"), y=5, label = "School re-opening")

# pdf("curve2.pdf", width = 12, height = 10)
r
# dev.off()
```
